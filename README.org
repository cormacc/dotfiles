...
| =agents/= | AI / coding agent config: pi extensions, prompts, skills. The =org-tasks= / =org-plan= / =org-jira= skills plus the =tasks= / =jira= / =leader-menu= / =emacsclient= extensions are also published as the =agent-org-memory= pi package (=packages.<system>.agent-org-memory= in this flake; see [[./agents/README.md]]). [[./agents.nix]] toggles between editable mode (default, full local suite) and packaged mode (collaborators, slice only) via =agents.mode=. |
